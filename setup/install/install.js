...
var QUESTION_ENVIRONMENT =
   "is this a production environment or local development [prod, dev]:";
...
var noRepeats = function(done) {
   fs.readFile(CONST_SETUP_FILE, function(err, data) {
      if (err) {
         ...
         AD.log();
         AD.log(
            "<yellow>WARN:</yellow> This install routine is only intended to be run once."
         );
         AD.log(
            "      If you need to make change to the roles and permissions, do that within "
         );
...
var questions = function(done) {
   AD.log();
   AD.log(
      "We will attempt to setup an initial admin user and permissions that"
   );
   AD.log("will be allowed to administrate system wide Roles and Permissions.");
   AD.log();
   AD.log("Keep in mind:");
   AD.log(
      "   <green>userid:</green>   needs to match what is returned via your"
   );
   AD.log(
      '             chosen authentication system.  So if you choose "admin" '
   );
   AD.log(
      "             as your userid, and you are using CAS authentication, "
   );
   AD.log(
      '             you will need to be able to login as "admin" under CAS.'
   );
   AD.log();
   ...
   SiteMultilingualLanguage.find({})
      .then(function(list) {
         if (!list || list.length < 1) {
            ...
            AD.log();
            AD.log(
               "Type in a comma separated list of language definitions to create."
            );
            AD.log(
               "Each definition should be in the format: <yellow>i18n code</yellow>:<green>utf8 label</green>"
            );
            AD.log("");
            AD.log(
               "    For example, if you wanted to create English, Korean, and Simplified Chinese, "
            );
            AD.log(
               "    type: <yellow>en</yellow>:<green>English</green>,<yellow>ko</yellow>:<green>Korean</green>,<yellow>zh-hans</yellow>:<green>中文</green>"
            );
            AD.log();
            ...
            AD.log(
               "<green>confirm:</green> there are languages installed in the site."
            );
            done();
         }
         return null;
      ...
      .catch(function(err) {
         if (err.code == "ER_NO_SUCH_TABLE") {
            AD.log(
               "<red>bad:</red> it appears the site tables have not been created in our default DB."
            );

            if (Data.isProduction) {
               AD.log(
                  "<yellow>do :</yellow> be sure to properly reference the correct DB, or create the tables in this one."
               );
            } else {
               AD.log(
                  "<yellow>do :</yellow> you can have sails create them for you:"
               );
               AD.log("<yellow>  1:</yellow> cd up/to/your/sails/directory");
               AD.log("<yellow>  2:</yellow> vi config/models.js");
               AD.log(
                  "<yellow>  3:</yellow> change migrate:<yellow>'safe'</yellow> to migrate:<yellow>'alter'</yellow>"
               );
      ...
      if (err) {
         ...
         langs.forEach(function(def) {
            ...
            if (parts.length == 2) {
               ...
               SiteMultilingualLanguage.create({
                  language_code: code,
                  language_label: label
               })
                  .then(function() {
                     AD.log(
                        "<green>created:</green> language reference <yellow>" +
                           code +
                           "</yellow> : <green>" +
                           label +
                           "</green>"
...
var createUser = function(done) {
   SiteUser.find({ username: Data.admin }).then(function(user) {
      // AD.log('... user:', user);
      if (user.length == 0) {
         ...
         AD.log(
            "<yellow>found  :</yellow> existing user account: <yellow>" +
               user[0].username +
               " (" +
               user[0].guid +
               ")</yellow>"
...
var createRole = function(done) {
   ...
   PermissionRoleTrans.find({ role_label: Data.role }).then(function(
      roleTrans
   ) {
      ...
      if (roleTrans.length == 0) {
         ...
         Multilingual.model
            .create({
               ...
               data: {
                  ...
                  language_code: lang,
                  role_description: "System Wide Administrator Role"
               }
            })
            .fail(function(err) {
               done(err);
         ...
         AD.log(
            "<yellow>found  :</yellow> existing role: <yellow>" +
               roleTrans[0].role_label +
               " (" +
               roleTrans[0].role_description +
               ")</yellow>"
         );
...
var createScope = function(done) {
   ...
   PermissionScopeTrans.find({ label: Data.scope }).then(function(scope) {
      // AD.log('... roleTrans:', roleTrans);
      ...
      if (scope.length == 0) {
            ...
            [
               ...
               function(next) {
                  PermissionScopeObject.find({ keyModel: "siteuser" })
                     .then(function(user) {
                        if (user.length == 0) {
                           PermissionScopeObject.createMultilingual({
                              keyModel: "siteuser",
                              name: "Site User"
                           })
               ...
               function(next) {
                  AD.log(
                     "<green>create :</green> creating new scope: " + Data.scope
                  );
                  var filterUI = {
                     condition: "AND",
                     rules: [
                  ...
                  PermissionScope.createMultilingual({
                     label: Data.scope,
                     object: newScopeObject.id,
                     filterUI: filterUI
                  })
         ...
         AD.log(
            "<yellow>found  :</yellow> existing scope: <yellow>" +
               scope[0].label +
               "</yellow>"
         );
...
var verifyActions = function(done) {
   ...
   var defaultOpsActions = require(path.join(
      __dirname,
      "..",
      "permissions",
      "actions.js"
   ...
   PermissionAction.find({ action_key: actionKeys }).then(function(actions) {
      if (actions.length < actionKeys.length) {
         ...
         actionKeys.forEach(function(key) {
            AD.log(
               "<red>missing:</red> verifyActions(): could not find action key: <yellow>" +
                  key +
                  " </yellow>"
            );
         });
         ...
         AD.log();
         AD.log(
            "<yellow>Before you run this script, make sure you run:</yellow>"
         );
         AD.log("    <green>npm install appdevdesigns/appdev-core</green>");
         AD.log(
            "    <green>npm install appdevdesigns/appdev-opsportal</green>"
         );
         AD.log();
...
var associateRoleToActions = function(done) {
   if (ActionKeys.length == 0) {
      AD.log(
         "<yellow>warn:</yellow> no action keys to link to role.  that doesn't sound right."
      );
      done();
   } else {
      ...
      AdminRole.save()
         ...
         .catch(function(err) {
            AD.log(
               "<yellow>linked :</yellow> action keys already linked to role"
            );
      ...
      .then(function(perm) {
         ...
         perm
            ...
            .then(function(newPerm) {
               AD.log(
                  "<green>created:</green> permission assignment for admin user + role + scope"
               );
               done();
               return null;
            })
...
function verifyInstanceActions(def, toolInstance, done) {
   ...
   if (actionsToAdd.length == 0) {
      ...
      PermissionAction.find({ action_key: actionsToAdd }).exec(function(
         err,
         foundActions
      ) {
         if (err) {
...
var createToolDef = function(def, done) {
   OPConfigToolDefinition.create(def).exec(function(err, tool) {
      if (err) {
...
var markComplete = function(done) {
   ...
   fs.writeFile(CONST_SETUP_FILE, "done", function(err) {
      if (err) {
         AD.log(
            "<red>error  :</red> unable to write " + CONST_SETUP_FILE + " file."
         );
         done(err);
      } else {
