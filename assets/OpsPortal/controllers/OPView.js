               ...
               AD.Control.OpsTool.extend("OPView", {
                     ...
                     if (this.options.url != "") {
                        AD.comm.service
                           ...
                           .fail(function(err) {
                              ...
                              AD.error.log(
                                 "Error loading OPView url:" +
                                    _this.options.url,
                                 { url: _this.options.url, error: err }
                              );
                           ...
                           .then(function(viewData) {
                              ...
                              viewData.objects.forEach(function(object) {
                                 if (object.key && object.path) {
                                    objectsReady.push(
                                       AD.Model.ready(object.key)
                                    );
                                    steal(object.path); // loads the model
                                 } else {
                                    AD.error.log(
                                       "Invalid definition for an object provided.",
                                       {
                                          object: object,
                                          viewdata: viewData,
                              ...
                              $.when
                                 ...
                                 .fail(function(err) {
                                    AD.error.log(
                                       "Error loading OPView objects:" +
                                          _this.options.url,
                                       { objects: viewData.objects, error: err }
                                    );
                                    _this.dfdReady.reject(err);
                                 })
                                 .then(function() {
                                    ...
                                    viewData.controller =
                                       viewData.controller || [];
                                    viewData.controller.forEach(function(
                                       controller
                                    ) {
                                       if (controller.key && controller.path) {
                                          controllersReady.push(
                                             AD.Control.ready(controller.key)
                                          );
                                          steal(controller.path);
                                       } else {
                                          AD.error.log(
                                             "Invalid definition for a controller provided.",
                                             {
                                    ...
                                    $.when
                                       .apply($, controllersReady)
                                       .fail(function(err) {
                                          AD.error.log(
                                             "Error loading OPView controller:" +
                                                _this.options.url,
                                             {
                                                ...
                                                var init = $.extend(
                                                   {
                                                      id: controller.elID,
                                                      areaKey:
                                                         _this.options.areaKey,
                                                      toolKey:
                                                         _this.options.toolKey
                                                   },
                                                   controller.init
                                                );
                                                _this.options;

                                                _this.controllers.push(
                                                   AD.Control.new(
                                                      controller.key,
                                                      $el,
                                                      init
                        ...
                        console.error(
                           "... OPView.init() : no url provided to this OPView!"
                        );
                     }
                  },
                  ...
                  initDOM: function(arrayControllers) {
                     ...
                     arrayControllers.forEach(function(controller) {
                        ...
                        var id = AD.util.uuid(
                           AD.util.string.replaceAll(controller.key, ".", "_")
                        );
                        div += '<div id="' + id + '"></div>';
                        controller.elID = id;
                     });
