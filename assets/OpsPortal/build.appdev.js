         ...
         [
            ...
            function(next) {
               fs.exists(path.join(process.cwd(), "appdev"), function(exists) {
                  if (exists) {
                     ...
                     AD.log(
                        "<red>error:</red> building OpsPortal requires appdev module to be installed as well."
                     );
                     var err = new Error(
                        "building OpsPortal requires appdev module to be installed as well. "
                     );
                     next(err);
                  }
               });
            ...
            function(next) {
               ...
               build(
                  ...
                  .catch(function(err) {
                     AD.log.error(
                        "<red>could not complete OpsPortal JS build!</red>",
                        err
                     );
                     next(err);
                  });
            ...
            function(next) {
               AD.log(
                  "<green>cleaning</green> the OpsPortal/production.js file"
               );

               var patches = [
                  {
                     file: path.join("OpsPortal", "production.js"),
                     tag: 'id:"packages/OpsPortal-appdev.js",',
                     replace: 'id:"appdev/production.js",'
                  }
               ];
