            ...
            .then(function() {
               ...
               AD.Control.OpsTool.extend("RBAC", {
                  ...
                  initDOM: function() {
                     ...
                     var controllers = {
                        "opstools.RBAC.Users": {
                           el: "." + _this.uuid("rbac-users"),
                           opt: { uuid: this.data.uuid }
                        },
                        "opstools.RBAC.Roles": {
                           el: "." + _this.uuid("rbac-roles"),
                           opt: { uuid: this.data.uuid }
                        },
                        "opstools.RBAC.Scopes": {
                           el: "." + _this.uuid("rbac-scopes"),
                           opt: { uuid: this.data.uuid }
                        }
                     };
                  ...
                  loadUsers: function() {
                     ...
                     User.findAll()
                        .fail(function(err) {
                           AD.error.log("RBAC:RBAC: Error loading Users", {
                              error: err
                           });
                        })
                        .then(function(list) {
                  ...
                  loadData: function() {
                     ...
                     Roles.findAll()
                        .fail(function(err) {
                           AD.error.log("RBAC:RBAC: Error loading Roles", {
                              error: err
                           });
                        })
                        .then(function(list) {
                     ...
                     Scopes.findAll()
                        .fail(function(err) {
                           AD.error.log("RBAC:RBAC: Error loading scopes", {
                              error: err
                           });
                        })
                        .then(function(list) {
                     ...
                     if (this.element) {
                        ...
                        for (var p in this.portals) {
                           ...
                           if (p.toLowerCase() == portalKey.toLowerCase()) {
                              ...
                              if (
                                 this.data.resize &&
                                 this.data.resize.available
                              ) {
                                 if (this.portals[p].resize) {
                                    this.portals[p].resize({
                                       height: this.data.resize.available
                                    });
                                 }
                              }
                     ...
                     if (this.element) {
                        ...
                        if (!this.data.resize.menuBar) {
                           var selectedMenuItem = this.element.find(
                              "li.selected.rbac-menu"
                           );
                           this.data.resize.menuBar = selectedMenuItem.outerHeight(
                              true
                           );
                        }
                        ...
                        this.data.resize.available =
                           totalHeight - this.data.resize.menuBar;
