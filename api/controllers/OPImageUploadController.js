...
// console.log('... options:', options);
			...
			function(next) {
				...
				sails.log.silly('--- OPImageUploadController.createBase64(): make sure destination directory exists')
				fs.stat(destPath, function(err, stat){
					if (err && err.code === 'ENOENT') {
						...
						sails.log.silly('--- OPImageUploadController.createBase64(): directory EXISTS!');
			...
			function(next) {
				...
				fs.writeFile(tempPath, binaryData, (err)=>{
					if (err) {
						ADCore.error.log('OPImageUploadController.createBase64(): unable to write to file ['+tempPath+']', {error:err, filePath:tempPath });
						err.code = 500;
						next(err);
					}
					fileRef = tempPath;
			...
			//    and also save it in our destination folder:
			function(next) {
				sails.log.silly('--- OPImageUploadController.createBase64(): jimp rotate');
                ...
                .then(function(image){
                    image.write(destRef, function(err){
                        ...
                        if (err) {
                        	ADCore.error.log('OPImageUploadController.createBase64(): jimp: error writing file ['+destRef+']', {error:err, fileRef:fileRef, destRef:destRef });
                        	err.code = 500;
                            next(err);
                        } else {
                            next()
                ...
                .catch(function(err){
                	ADCore.error.log('OPImageUploadController.createBase64(): jimp: error manipulating file ['+fileRef+']', {error:err, fileRef:fileRef, destRef:destRef });
					err.code = 500;
                    next(err);
                });
			},
			...
			function(next) {
				sails.log.silly('--- OPImageUploadController.createBase64(): save OPImageUpload values');
				// uuid : the fileName without '.ext'
				uuid = fileName.split('.')[0];
				...
				.catch(function(err){
					ADCore.error.log('OPImageUploadController.createBase64(): OPImageUpload.create(): error creating image DB entry', {error:err});
					err.code = 500;
					next(err);
				})
			}
			...
			if (err) {
				...
				ADCore.error.log('OPImageUploadController.createBase64(): Error saving image', {error:err, options:options, tempPath:tempPath, destPath:destPath })
				res.AD.error(err, err.code);
			} else {
				sails.log.silly('--- OPImageUploadController.js: return response');
				// prepare our return data
    		...
    		function(next) {
    			...
    			fs.access(destFile, fs.constants.R_OK , function(err) {
					if (err) {
						...
						nError.code = 500;
						nError.err = err;
						console.error('... OPImageUploadController.read(): cannot access image file:'+destFile);
						next(nError);
					} else {
						next();
					}
    		...
    		function(next) {
    			...
    			fs.access(destFile, fs.constants.R_OK , function(err) {
					if (err) {
						...
						nError.code = 500;
						nError.err = err;
						console.error('... OPImageUploadController.readBase64(): cannot access image file:'+destFile);
						next(nError);
					} else {
						next();
					}
